...
(function () {
  ...
  function shell(title, eyebrow, body, controls) {
    host.innerHTML = '<div class="weekend-shell"><div class="weekend-top"><button class="btn btn--ghost btn--sm" id="weekend-back" type="button">← Missions</button><span class="weekend-score">⭐ ' + state.stars + ' <b>·</b> ⚡ ' + state.xp + ' Weekend XP</span></div><div class="weekend-progress" aria-label="Adventure progress"><i style="width:' + progress() + '%"></i></div><div class="weekend-card"><p class="weekend-eyebrow">' + eyebrow + '</p><h2 id="weekend-title">' + title + '</h2>' + body + (controls ? '<div class="weekend-controls">' + controls + '</div>' : '') + '</div></div>';
    document.getElementById("weekend-back").addEventListener("click", back);
  }
  function reward(message) { state.xp += 20; state.stars++; save(); var box = host.querySelector(".weekend-feedback"); if (box) box.innerHTML = '<span aria-hidden="true">🎉</span> ' + message + ' <b>+20 XP</b>'; }
  ...
  function keyChallenge() {
    ...
    host.querySelectorAll("[data-key]").forEach(function (btn) { btn.onclick = function () { if (btn.dataset.key !== target) { host.querySelector(".weekend-feedback").textContent = "🕵️ That clue is elsewhere — inspect the keyboard again!"; return; } btn.classList.add("is-found"); state.seen[target] = true; reward("That key was hiding in plain sight!"); setTimeout(keyChallenge, 520); }; });
  }
  ...
  function shortcutChallenge(index) {
    ...
    shell("Shortcut Hero", "Clue 3 of 5 · Scene " + (index + 1) + "/" + shortcuts.length, '<div class="scenario-card"><span aria-hidden="true">💻</span><p>' + q[0] + '</p></div><p class="weekend-feedback" role="status">Which helpful move would you use?</p><div class="choice-grid">' + options.map(function (o) { return '<button class="shortcut-choice" type="button" data-answer="' + o + '">' + o + '</button>'; }).join("") + '</div>', '');
  ...
  function mythChallenge(index) {
    ...
    var q = myths[index];
    shell("Truth or myth?", "Clue 4 of 5 · " + (index + 1) + "/" + myths.length, '<div class="myth-card"><span aria-hidden="true">🔍</span><p>“' + q[0] + '”</p></div><p class="weekend-feedback" role="status">Trust your computer detective instincts.</p><div class="truth-buttons"><button type="button" data-value="true">✓ Truth</button><button type="button" data-value="false">✕ Myth</button></div>', '');
  ...
  function bossChallenge(index) {
    ...
    shell("Repair the computer", "Boss challenge · Repair " + (index + 1) + "/5", '<div class="repair-meter" aria-label="Computer repair progress"><span style="width:' + (index / 5 * 100) + '%"></span></div><div class="repair-scene"><div class="lab-computer" aria-hidden="true">🖥️</div><p><b>' + q[0] + '</b> is flickering. ' + q[1] + '</p></div><p class="weekend-feedback" role="status">One quick fix at a time.</p><div class="choice-grid">' + options.map(function (o) { return '<button class="shortcut-choice" type="button" data-fix="' + o + '">' + o + '</button>'; }).join("") + '</div>', '');
    host.querySelectorAll("[data-fix]").forEach(function (b) { b.onclick = function () { if (b.dataset.fix !== q[2]) { host.querySelector(".weekend-feedback").textContent = "⚙️ That part still rattles. Try another repair clue!"; return; } state.bossIndex = index + 1; reward(q[0] + " repaired!"); setTimeout(function () { bossChallenge(index + 1); }, 520); }; });
  }
  function celebration() {
    state.done = true; save();
    shell("Case solved!", "The computer powers on", '<div class="celebration-burst" aria-hidden="true">✨ 🖥️ ✨</div><p class="weekend-lead">The lab computer glows back to life. Principal WEB79 salutes <b>' + escape(student()) + '</b>: you found every clue and repaired every part.</p><div class="unlock-card"><span aria-hidden="true">🏅</span><div><b>Weekend Badge unlocked</b><small>Keyboard Detective</small></div></div><p class="weekend-dialogue">“You revised a whole week of computer skills without even noticing. That is expert detective work.”</p>', '<button class="btn btn--primary btn--lg" id="weekend-cert" type="button">Reveal my certificate →</button>');
    document.getElementById("weekend-cert").onclick = function () { advance("certificate"); };
  }
  function certificate() {
    shell("Keyboard Detective Certificate", "Weekend Treat complete", '<div class="weekend-certificate"><div aria-hidden="true">🏆</div><p>This certifies that</p><h3>' + escape(student()) + '</h3><p>has solved the WEB79 computer-lab mystery and earned the title</p><h4>Keyboard Detective</h4><p class="cert-small">Weekend XP earned: ' + state.xp + ' · Issued ' + new Date().toLocaleDateString(undefined, { month: "long", day: "numeric", year: "numeric" }) + '</p></div>', '<button class="btn btn--ghost" id="weekend-print" type="button">Print certificate</button><button class="btn btn--primary" id="weekend-return" type="button">Back to missions</button>');
